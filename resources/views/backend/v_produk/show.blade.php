                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror" disabled>
                                    ...
                                    <option value="{{ $row->id }}" {{ old('kategori_id', $show->kategori_id) == $row->id ? 'selected' : '' }}>{{ $row->nama_kategori }}
                                    </option>
                                    @endforeach
                                </select>
                                @error('kategori_id')
                        ...
                        <div class="col-md-6">
                            ...
                            <div id="foto-container">
                                <div class="row">
                                    ...
                                    @empty
                                    <div class="col-12">
                                        <p>Tidak ada foto produk yang tersedia.</p>
                                    </div>
                                    @endforelse
                                </div>
                                <br>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addFotoButton.addEventListener('click', function() {
            ...
            fotoRow.innerHTML = '<form action = "{{ route('backend.foto_produk.store') }}" method = "post" enctype = "multipart/form-data" >@csrf <div class = "col-md-12" > <input type="hidden" name="produk_id" value="{{ $show->id }}"><input type="file" name="foto_produk[]" class="form-control  @error('foto_produk') is-invalid @enderror"><button type="submit" class="btn btn-success">Simpan</button> </div> </form>';
